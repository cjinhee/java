...
public class Ex26_DataTime {
	...
	private static void m7() {
		...
		//- 시간 + 시간 = 시간
		//- 시간 - 시간 = 시간
		//시간단위를 구할 땐 Calendar를 사용하지 않고 정수타입을 사용함
		
		//점심: 2시간
		//Calendar a = Calendar.getInstance();
		//a.set(Calendar.HOUR, 2);
		//System.out.printf("%tF %tT\n", a, a); //2023-07-21 02:39:46 의미가 다른 2시간임
		...
		//2시간 40분 + 30분 = 2시간 70분 = 3시간 10분 => 자리 올림을 잘하라
	...
	private static void m6() {
		...
		//- epoch time
		//- tick       --두가지로 부름
		long nowTick = now.getTimeInMillis(); //리턴값이 long
		System.out.println(nowTick); //1689898946561ms - 밀리세컨드 ,기준점으로 흘러간 누적된 시간
		
		long endTick = end.getTimeInMillis();
		System.out.println(endTick); //1703636728157
	...
	private static void m5() {
		...
		//읽기 + 출력
		System.out.println(now.get(Calendar.YEAR));        //이것과 아래 중 하나만 사용해도 됨
	...
	private static void m2() {
		...
		//java.util.GregorianCalendar[time=1689838493114,areFieldsSet=true,areAllFieldsSet=true,lenient=true,zone=sun.util.calendar.ZoneInfo[id="Asia/Seoul",offset=32400000,dstSavings=0,useDaylight=false,transitions=30,lastRule=null],firstDayOfWeek=1,minimalDaysInFirstWeek=1,ERA=1,YEAR=2023,MONTH=6,WEEK_OF_YEAR=29,WEEK_OF_MONTH=4,DAY_OF_MONTH=20,DAY_OF_YEAR=201,DAY_OF_WEEK=5,DAY_OF_WEEK_IN_MONTH=3,AM_PM=1,HOUR=4,HOUR_OF_DAY=16,MINUTE=34,SECOND=53,MILLISECOND=114,ZONE_OFFSET=32400000,DST_OFFSET=0]
		...
		System.out.println(c1.get(Calendar.DAY_OF_YEAR));  //201 > 일(올해 들어 몇일째인지)
		System.out.println(c1.get(Calendar.DAY_OF_MONTH)); //20 > 일(이번달 들어 몇일째인지)
		System.out.println(c1.get(Calendar.DAY_OF_WEEK));  //5 > 요일 > 1(일)~7(토)
		System.out.println(c1.get(Calendar.WEEK_OF_YEAR)); //29 > 주 (올해 들어 몇주째인지)
		System.out.println(c1.get(Calendar.WEEK_OF_MONTH));//4 > 주 (이번달 들어 몇주째인지)
		System.out.println();
		...
		System.out.printf("지금은 %d시 %02d분 %02d초입니다.", c1.get(Calendar.HOUR_OF_DAY), c1.get(Calendar.MINUTE), c1.get(Calendar.SECOND) );
		System.out.println();
		System.out.printf("지금은 %s %d시 %02d분 %02d초입니다.", c1.get(Calendar.AM_PM) == 0 ? "오전" : "오후", c1.get(Calendar.HOUR), c1.get(Calendar.MINUTE), c1.get(Calendar.SECOND) );
		System.out.println();
